...
def calculate_grade(
    ...
    return f"Final Grade: {letter_grade(total=ppp_components(ex=ex, rd=rd, ls=ls, cl=cl) 
                                        + mastery_components(qz=qz,fn=fn), has_min_mastery = has_min_mastery(qz=qz, fn=fn))} ({round((ppp_components(ex=ex, rd=rd, ls=ls, cl=cl) 
                                                                                                                                      + mastery_components(qz=qz, fn=fn))) * 100}%)"
